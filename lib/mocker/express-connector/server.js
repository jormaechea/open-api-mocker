...
class Server {
  ...
  async init() {
    return (app) => {
      if (app !== this._lastConenctedApp) {
        app.use(this.middleware.handle());
      }
      ...
      this.paths.map((path) => {
        ...
        router[expressHttpMethod](uris, (req, res) => {
          ...
          const { query, params, headers, cookies, body: requestBody } = req;
