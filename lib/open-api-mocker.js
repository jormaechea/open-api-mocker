...
class OpenApiMocker {
	...
	setSchema(schema) {
		...
		if(this.schemaLoader && this.options.watch) {
			if(this.schemaLoader.unwatch)
				this.schemaLoader.unwatch();
			this.schemaLoader.removeAllListeners();
			this.options.alreadyWatching = false;
		}
