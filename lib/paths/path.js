...
class Path {
	...
	validateRequestBody(requestBody) {
		...
		try {
			const error = this.isValidType(schema.type, requestBody);

			if(error)
				return [`Invalid request body. Expected value of type ${schema.type} but received ${util.inspect(requestBody)}`];

		} catch(e) {
			return [`${e.message} for request body`];
		}
