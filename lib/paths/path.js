...
class Path {
	...
	async getResponse(preferredStatusCode, preferredExampleName, preferredRate, preferredLatency) {

		const applyExperiment = preferredRate ? this.getRandomInt(100) < (preferredRate) : true;
		...
		const {
			...
			schema,
			responseMimeType
		} = preferredStatusCode && applyExperiment ? this.getResponseByStatusCode(preferredStatusCode) : this.getFirstResponse();
